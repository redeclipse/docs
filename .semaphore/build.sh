    ...
    for i in *; do
        if [ -d "${i}" ]; then
            if [ "${i}" != "src" ] && [ "${i}" != "bits" ]; then
                pushd "${i}" || return 1
                for j in *; do
                    if [ ! -d "${j}" ] &&  [ ! -e "${SEMABUILD_GIT}/${i}/${j}" ]; then
                        git rm "${j}" || return 1
                    fi
                done
                popd || return 1
            fi
        elif [ ! -e "${SEMABUILD_GIT}/${i}" ]; then
            git rm "${i}" || return 1
        fi
    done
    return 0
...
semabuild_update() {
    ...
    git commit -a -m "Build docs:${SEMAPHORE_WORKFLOW_NUMBER} from ${SEMAPHORE_GIT_SHA}" || return 1
